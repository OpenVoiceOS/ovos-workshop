...
The real munged path exercises three §5/§6/§7 behaviours the direct unit
tests (which bypass ``munge_intent_parser`` and skill-level resource loading)
do not: ``excluded`` vocab must survive munging (§5.2), the template
``intent_name`` must drop the ``.intent`` suffix (§6), and the ``entity_name``
must drop the hash-munged resource id (§7). The producer un-munges all three
in its spec emission path so the wire payloads carry clean names.
"""
import sys
