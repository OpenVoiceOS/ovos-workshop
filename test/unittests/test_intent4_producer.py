...
registration topics alongside the legacy ones (dual-emit, because the
consolidation is an N->1 mapping that the bus cannot transparently
...
class RealMungedFlowSpecTest(unittest.TestCase):
    ...
    assert the emitted spec payload carries the clean values.

    OVOSSkill munges names before calling IntentServiceInterface; the spec
    payload must un-munge them so consumers see clean intent/entity/vocab
    names on the wire.
    """
