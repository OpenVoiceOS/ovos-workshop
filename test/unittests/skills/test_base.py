...
class TestOVOSSkill(unittest.TestCase):
    ...
    def test_set_cross_skill_context_invalid_word_leaves_session_unchanged(self):
        """A non-string `word` must raise before the session write, not
        after it. Writing first and validating later (in the legacy
        `handle_set_cross_context` listener) would leave a bad entry in
        the shared session even though the call raised."""
        bus = FakeBus()
        skill = OVOSSkill(bus=bus, skill_id="skill.a")

        session = Session("test_set_cross_skill_context_invalid_word")
        msg = Message("some.intent", {}, {"session": session.serialize()})

        def dispatch(message):
            with self.assertRaises(ValueError):
                skill.set_cross_skill_context("thing", None)

        dispatch(msg)

        try:
            live = SessionManager.get(msg)
            self.assertNotIn("thing", live.intent_context)
        finally:
            skill.default_shutdown()
